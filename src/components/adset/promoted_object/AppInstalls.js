...
// 根据系列选择的目标 展示不同内容
const AppInstalls = React.memo(
  ({ application, dispatch, adaccount_id, handleSubmit }) => {
    ...
    return (
      <div>
        ...
        <Select
          ...
          showSearch
          placeholder="选择账户绑定的应用(备注: 跨账号复制时 非应用系列请忽略此选项), 未见app可搜索"
        ...
        >
          {options
            .filter(
              item =>
                item.app_install_tracked && item.object_store_urls[appStore]
            )
            .map(d => (
